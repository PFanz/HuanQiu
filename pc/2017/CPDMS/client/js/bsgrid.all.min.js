...
$.bsgrid = {
  ...
  getKeysString: function(c, a) {
    ...
    if (c instanceof Array) {
      $.each(c, function(f, e) {
        if ((d.toString() + a).indexOf(a + e.name + a) == -1) { d.append(a + e.name) }
      })
    } else {
      for (var b in c) { d.append(a + b) }
    }
    return d.length > 0 ? d.toString().substring(1) : ""
  },
  forcePushPropertyInObject: function(b, a, c) {
    if (b.hasOwnProperty(a)) { b[a + "_f"] = c } else { b[a] = c }
  },
  adaptAttrOrProp: function(b, d, c) {
    ...
    if (c == undefined) {
      ...
      if (a >= 16) { b.prop(d, c) } else { b.attr(d, c) }
    }
  },
...
(function(a) {
  a.fn.bsgrid_paging = {
    defaults: { loopback: false, pageSize: 20, pageSizeSelect: false, pageSizeForGrid: [5, 10, 20, 25, 50, 100, 200, 500], pageIncorrectTurnAlert: true, pagingLittleToolbar: false, pagingBtnClass: "pagingBtn" },
    pagingObjs: {},
    init: function(h, g) {
      var d = { settings: a.extend(true, {}, a.fn.bsgrid_paging.defaults, g), pagingId: h, totalRowsId: h + "_totalRows", totalPagesId: h + "_totalPages", curPageId: h + "_curPage", gotoPageInputId: h + "_gotoPageInput", gotoPageId: h + "_gotoPage", refreshPageId: h + "_refreshPage", pageSizeId: h + "_pageSize", firstPageId: h + "_firstPage", prevPageId: h + "_prevPage", nextPageId: h + "_nextPage", lastPageId: h + "_lastPage", startRowId: h + "_startRow", endRowId: h + "_endRow", totalRows: 0, totalPages: 0, curPage: 1, curPageRowsNum: 0, startRow: 0, endRow: 0 };
      if (g.pageSizeForGrid != undefined) { d.settings.pageSizeForGrid = g.pageSizeForGrid }
      var b = {
        options: d,
        page: function(i) { a.fn.bsgrid_paging.page(i, d) },
        getCurPage: function() {
          return a.fn.bsgrid_paging.getCurPage(d)
        },
        refreshPage: function() { a.fn.bsgrid_paging.refreshPage(d) },
        firstPage: function() { a.fn.bsgrid_paging.firstPage(d) },
        prevPage: function() { a.fn.bsgrid_paging.prevPage(d) },
        nextPage: function() { a.fn.bsgrid_paging.nextPage(d) },
        lastPage: function() { a.fn.bsgrid_paging.lastPage(d) },
        gotoPage: function(i) { a.fn.bsgrid_paging.gotoPage(d, i) },
        createPagingToolbar: function() {
          return a.fn.bsgrid_paging.createPagingToolbar(d)
        },
        setPagingToolbarEvents: function() { a.fn.bsgrid_paging.setPagingToolbarEvents(d) },
        dynamicChangePagingButtonStyle: function() { a.fn.bsgrid_paging.dynamicChangePagingButtonStyle(d) },
        setPagingValues: function(j, i) { a.fn.bsgrid_paging.setPagingValues(j, i, d) }
      };
      a.fn.bsgrid_paging.pagingObjs[h] = b;
      a("#" + h).append(b.createPagingToolbar());
      if (d.settings.pageSizeSelect) {
        if (a.inArray(d.settings.pageSize, d.settings.pageSizeForGrid) == -1) { d.settings.pageSizeForGrid.push(d.settings.pageSize) }
    ...
    refreshPage: function(b) { a.fn.bsgrid_paging.page(a.fn.bsgrid_paging.getCurPage(b), b) },
    ...
    gotoPage: function(b, c) {
      if (c == undefined) { c = a("#" + b.gotoPageInputId).val() }
      if (a.trim(c) == "" || isNaN(c)) { a.fn.bsgrid_paging.alert(a.bsgridLanguage.needInteger) } else {
        if (parseInt(c) < 1 || parseInt(c) > b.totalPages) { a.fn.bsgrid_paging.alert(a.bsgridLanguage.needRange(1, b.totalPages)) } else {
    ...
    incorrectTurnAlert: function(b, c) {
      if (b.settings.pageIncorrectTurnAlert) { a.fn.bsgrid_paging.alert(c) }
    },
    alert: function(c) {
      try { a.bsgrid.alert(c) } catch (b) { alert(c) }
    },
    createPagingToolbar: function(b) {
      ...
      e.append("<tr>");
      if (b.settings.pageSizeSelect) { e.append("<td>" + a.bsgridLanguage.pagingToolbar.pageSizeDisplay(b.pageSizeId, c) + "</td>") }
    ...
    setPagingToolbarEvents: function(b) {
      ...
      a("#" + b.firstPageId).click(function() { a.fn.bsgrid_paging.firstPage(b) });
      a("#" + b.prevPageId).click(function() { a.fn.bsgrid_paging.prevPage(b) });
      a("#" + b.nextPageId).click(function() { a.fn.bsgrid_paging.nextPage(b) });
      a("#" + b.lastPageId).click(function() { a.fn.bsgrid_paging.lastPage(b) });
      a("#" + b.gotoPageInputId).keyup(function(c) {
        if (c.which == 13) { a.fn.bsgrid_paging.gotoPage(b) }
      });
      a("#" + b.gotoPageId).click(function() { a.fn.bsgrid_paging.gotoPage(b) });
      a("#" + b.refreshPageId).click(function() { a.fn.bsgrid_paging.refreshPage(b) })
    },
...
(function($) {
  $.fn.bsgrid = {
    ...
    init: function(gridId, settings) {
      ...
      if ($("#" + gridId).find("tbody").length == 0) { $("#" + gridId + " thead").after("<tbody></tbody>") }
      if ($("#" + gridId).find("tfoot").length == 0) { $("#" + gridId).append('<tfoot style="display: none;"></tfoot>') }
      options.columnsModel = $.fn.bsgrid.initColumnsModel(options);
      if (settings.pageSizeForGrid != undefined) { options.settings.pageSizeForGrid = settings.pageSizeForGrid }
      options.settings.dataType = options.settings.dataType.toLowerCase();
      if (options.settings.pageSizeSelect) {
        if ($.inArray(options.settings.pageSize, options.settings.pageSizeForGrid) == -1) { options.settings.pageSizeForGrid.push(options.settings.pageSize) }
      ...
      var gridObj = {
        ...
        page: function(curPage) { $.fn.bsgrid.page(curPage, options) },
        search: function(params) { $.fn.bsgrid.search(params, options) },
        loadGridData: function(dataType, gridData) { $.fn.bsgrid.loadGridData(dataType, gridData, options) },
        getRows: function() {
        ...
        sort: function(obj) { $.fn.bsgrid.sort(obj, options) },
        ...
        appendHeaderSort: function() { $.fn.bsgrid.appendHeaderSort(options) },
        setGridBlankBody: function() { $.fn.bsgrid.setGridBlankBody(options) },
        createPagingOutTab: function() { $.fn.bsgrid.createPagingOutTab(options) },
        clearGridBodyData: function() { $.fn.bsgrid.clearGridBodyData(options) },
        ...
        refreshPage: function() { $.fn.bsgrid.refreshPage(options) },
        firstPage: function() { $.fn.bsgrid.firstPage(options) },
        prevPage: function() { $.fn.bsgrid.prevPage(options) },
        nextPage: function() { $.fn.bsgrid.nextPage(options) },
        lastPage: function() { $.fn.bsgrid.lastPage(options) },
        gotoPage: function(goPage) { $.fn.bsgrid.gotoPage(options, goPage) },
        initPaging: function() {
          return $.fn.bsgrid.initPaging(options)
        },
        setPagingValues: function() { $.fn.bsgrid.setPagingValues(options) }
      };
      ...
      if (!options.settings.pageAll) {
        ...
        try {
          ...
          $(window).resize(function() { $("#" + options.pagingOutTabId).width($("#" + gridId).width()) })
        } catch (e) {}
      }
      if (options.settings.isProcessLockScreen) { $.fn.bsgrid.addLockScreen(options) }
      try { $.fn.bsgrid.extendInitGrid.initGridExtendOptions(gridId, options) } catch (e) {}
      for (var key in options.settings.extend.initGridMethods) { options.settings.extend.initGridMethods[key](gridId, options) }
      if (options.settings.autoLoad) { setTimeout(function() { gridObj.page(1) }, 10) } else { gridObj.setGridBlankBody() }
      return gridObj
    },
    ...
    buildJsonData: {
      gridData: function(curPage, data) {
        return { success: true, totalRows: data.length, curPage: curPage, data: data }
      }
    },
    parseJsonData: {
    ...
    getPageCondition: function(curPage, options) {
      ...
      if (options.otherParames == false) {} else {
        if ((typeof options.otherParames).toLowerCase() == "string" || options.otherParames instanceof String) { params.append("&" + options.otherParames) } else {
          if (options.otherParames instanceof Array) { $.each(options.otherParames, function(i, objVal) { params.append("&" + objVal.name + "=" + objVal.value) }) } else {
            for (var key in options.otherParames) { params.append("&" + key + "=" + options.otherParames[key]) }
          }
        }
      }
    ...
    page: function(curPage, options) {
      ...
      if (options.settings.localData != false) {
        if (dataType == "json") { $.fn.bsgrid.loadGridData(dataType, $.fn.bsgrid.buildData.gridData(dataType, curPage, options.settings.localData), options) } else {
          if (dataType == "xml") { $.fn.bsgrid.loadGridData(dataType, "<xml>" + $.fn.bsgrid.buildData.gridData(dataType, curPage, options.settings.localData) + "</xml>", options) }
        }
        return
      }
      $.ajax({
        ...
        beforeSend: function(XMLHttpRequest) {
          if (options.settings.isProcessLockScreen) { $.fn.bsgrid.lockScreen(options) }
          options.settings.beforeSend(options, XMLHttpRequest)
        },
        complete: function(XMLHttpRequest, textStatus) {
          options.settings.complete(options, XMLHttpRequest, textStatus);
          if (options.settings.isProcessLockScreen) { $.fn.bsgrid.unlockScreen(options) }
        },
        success: function(data, textStatus) { 
          var gridData = {
            curPage: 1,
            success: 'true',
            totalRows: data.obj.rows.length,
            data: data.obj.rows
          }
          $.fn.bsgrid.loadGridData(dataType, gridData, options) 
        },
        error: function(XMLHttpRequest, textStatus, errorThrown) { $.fn.bsgrid.alert($.bsgridLanguage.errorForSendOrRequestData) }
      })
    },
    loadGridData: function(dataType, gridData, options) {
      var parseSuccess = $.fn.bsgrid.parseData.success(dataType, gridData);
      for (var key in options.settings.extend.beforeRenderGridMethods) { options.settings.extend.beforeRenderGridMethods[key](parseSuccess, gridData, options) }
      ...
      if (parseSuccess) {
        ...
        if (!options.settings.pageAll) { $.fn.bsgrid.setPagingValues(options) }
        ...
        $.fn.bsgrid.getRows(options).each(function(i) {
          var trObj = $(this);
          var record = null;
          if (i < curPageRowsNum && i < dataLen) { record = $.fn.bsgrid.parseData.getRecord(dataType, data, dataLen != totalRows ? i : startRow + i - 1) }
          $.fn.bsgrid.storeRowData(i, record, options);
          for (var key in options.settings.extend.renderPerRowMethods) { options.settings.extend.renderPerRowMethods[key](record, i, trObj, options) }
          options.settings.additionalRenderPerRow(record, i, trObj, options);
          for (var key in options.settings.event.customRowEvents) { trObj.bind(key, { record: record, rowIndex: i, trObj: trObj, options: options }, function(event) { options.settings.event.customRowEvents[key](event.data.record, event.data.rowIndex, event.data.trObj, event.data.options) }) }
          ...
          $(this).find("td").each(function(j) {
            ...
            if (i < curPageRowsNum && i < dataLen) {
              ...
              if (render != "") {
                ...
                if (index != "") {
                  ...
                  if (columnsModel[j].tip == "true") { $.fn.bsgrid.columnTip(this, value, record) }
                  if (options.settings.longLengthAotoSubAndTip) { $.fn.bsgrid.longLengthSubAndTip(this, value, columnsModel[j].maxLen, record) } else { tdObj.html(value) }
                }
              }
            } else { tdObj.html("&nbsp;") }
            for (var key in options.settings.extend.renderPerColumnMethods) {
              ...
              if (renderPerColumn_html != null && renderPerColumn_html != false) { tdObj.html(renderPerColumn_html) }
            }
            options.settings.additionalRenderPerColumn(record, i, j, tdObj, trObj, options);
            for (var key in options.settings.event.customCellEvents) { tdObj.bind(key, { record: record, rowIndex: i, colIndex: j, tdObj: tdObj, trObj: trObj, options: options }, function(event) { options.settings.event.customCellEvents[key](event.data.record, event.data.rowIndex, event.data.colIndex, event.data.tdObj, event.data.trObj, event.data.options) }) }
          })
        })
      } else { $.fn.bsgrid.alert($.bsgridLanguage.errorForRequestData) }
      for (var key in options.settings.extend.afterRenderGridMethods) { options.settings.extend.afterRenderGridMethods[key](parseSuccess, gridData, options) }
    ...
    addRowsClickEvent: function(options) {
      $.fn.bsgrid.getRows(options).filter(":lt(" + options.curPageRowsNum + ")").click(function() {
        if ($(this).hasClass("selected")) { $.fn.bsgrid.unSelectRow(options) } else { $.fn.bsgrid.selectRow($.fn.bsgrid.getRows(options).index($(this)), options) }
      })
    },
    ...
    selectRow: function(row, options) {
      ...
      if (options.settings.rowSelectedColor) { trObj.addClass("selected_color") }
      if (!!options.settings.event.selectRowEvent) { options.settings.event.selectRowEvent($.fn.bsgrid.getRowRecord(trObj), row, trObj, options) }
    },
    unSelectRow: function(options) {
      ...
      if (row != -1) {
        ...
        if (!!options.settings.event.unselectRowEvent) { options.settings.event.unselectRowEvent($.fn.bsgrid.getRowRecord(trObj), row, trObj, options) }
      }
    },
    getUserdata: function(options) { $("#" + options.gridId).data("userdata") },
    storeUserdata: function(userdata, options) { $("#" + options.gridId).data("userdata", userdata) },
    getRowRecord: function(rowObj) {
      var record = rowObj.data("record");
      return record == undefined ? null : record
    },
    storeRowData: function(row, record, options) { $.fn.bsgrid.getRow(row, options).data("record", record) },
    getAllRecords: function(options) {
      var records = [];
      $.fn.bsgrid.getRows(options).each(function() {
        var record = $.fn.bsgrid.getRowRecord($(this));
        if (record != null) { records[records.length] = record }
      });
      return records
    },
    ...
    sort: function(obj, options) {
      ...
      $.fn.bsgrid.getGridHeaderObject(options).each(function(i) {
        ...
        if (sortName != "") {
          ...
          if (!options.settings.multiSort && sortName != field) { $(this).find("a").attr("class", "sort sort-view") } else {
            if (sortName == field) {
              if (sortOrder == "") { sortOrder = "desc" } else {
                if (sortOrder == "desc") { sortOrder = "asc" } else {
    ...
    getSortOrder: function(obj, options) {
      ...
      if (sortOrder == "sort sort-view") { sortOrder = "" } else {
        if (sortOrder == "sort sort-asc") { sortOrder = "asc" } else {
          if (sortOrder == "sort sort-desc") { sortOrder = "desc" } else { sortOrder = "" }
        }
      }
      return sortOrder
    },
    ...
    appendHeaderSort: function(options) {
      ...
      $.fn.bsgrid.getGridHeaderObject(options).each(function(i) {
        if (columnsModel[i].sortName != "") {
          ...
          } else { sortHtml += "sort-view" }
          sortHtml += '">&nbsp;&nbsp;&nbsp;</a>';
          $(this).append(sortHtml).find(".sort").click(function() { $.fn.bsgrid.sort($(this).parent("th"), options) })
        }
      })
    },
    setGridBlankBody: function(options) {
      ...
      if (curPageRowsNum == 0) { rowsSb.append('<tr><td colspan="' + header.length + '">' + $.bsgridLanguage.noDataToDisplay + "</td></tr>") } else {
        for (var pi = 0; pi < curPageRowsNum; pi++) { rowsSb.append(rowSb) }
      }
      $("#" + options.gridId + " tbody").append(rowsSb.toString());
      if (curPageRowsNum != 0) {
        if (options.settings.stripeRows) { $.fn.bsgrid.getRows(options).filter(":even").addClass("even_index_row") }
        if (options.settings.rowHoverColor) { $("#" + options.gridId + " tbody tr").hover(function() { $(this).addClass("row_hover") }, function() { $(this).removeClass("row_hover") }) }
      }
      if (!options.settings.lineWrap) { $.fn.bsgrid.getRows(options).find("td").addClass("lineNoWrap") } else { $.fn.bsgrid.getRows(options).find("td").addClass("lineWrap") }
    },
    createPagingOutTab: function(options) {
      ...
      if (options.settings.pageAll) { pagingOutTabSb.append($.bsgridLanguage.noPagingation(options.noPagingationId) + "&nbsp;&nbsp;&nbsp;") }
      pagingOutTabSb.append("</td></tr></table>");
      $("#" + options.gridId).after(pagingOutTabSb.toString())
    },
    clearGridBodyData: function(options) { $.fn.bsgrid.getRows(options).find("td").html("&nbsp;") },
    ...
    columnTip: function(obj, value, record) { $(obj).attr("title", value) },
    alert: function(msg) {
      try { $.bsgrid.alert(msg) } catch (e) { alert(msg) }
    },
    longLengthSubAndTip: function(obj, value, maxLen, record) {
      var tip = false;
      if (value.length > maxLen) {
        try {
          if (value.indexOf("<") < 0 || value.indexOf(">") < 2 || $(value).text().length == 0) { tip = true }
        } catch (e) { tip = true }
      }
      if (tip) {
    ...
    refreshPage: function(options) {
      if (!options.settings.pageAll) { $.fn.bsgrid.getPagingObj(options).refreshPage() } else { $.fn.bsgrid.page(1, options) }
    },
    firstPage: function(options) { $.fn.bsgrid.getPagingObj(options).firstPage() },
    prevPage: function(options) { $.fn.bsgrid.getPagingObj(options).prevPage() },
    nextPage: function(options) { $.fn.bsgrid.getPagingObj(options).nextPage() },
    lastPage: function(options) { $.fn.bsgrid.getPagingObj(options).lastPage() },
    gotoPage: function(options, goPage) { $.fn.bsgrid.getPagingObj(options).gotoPage(goPage) },
    initPaging: function(options) {
      $("#" + options.pagingOutTabId + " td").attr("id", options.pagingId);
      return $.fn.bsgrid_paging.init(options.pagingId, { gridId: options.gridId, pageSize: options.settings.pageSize, pageSizeSelect: options.settings.pageSizeSelect, pageSizeForGrid: options.settings.pageSizeForGrid, pageIncorrectTurnAlert: options.settings.pageIncorrectTurnAlert, pagingLittleToolbar: options.settings.pagingLittleToolbar, pagingBtnClass: options.settings.pagingBtnClass })
    },
    setPagingValues: function(options) { $.fn.bsgrid.getPagingObj(options).setPagingValues(options.curPage, options.totalRows) }
  }
})(jQuery);
(function($) {
  ...
  $.extend(true, $.fn.bsgrid.defaults.colsProperties, { lineNumberAttr: "w_num", checkAttr: "w_check", editAttr: "w_edit", aggAttr: "w_agg" });
  ...
  $.fn.bsgrid.extendInitGrid.initGridExtendOptions = function(gridId, options) {
    ...
    if ($.fn.bsgrid.getGridHeaderObject(options).filter("[" + colsProperties.lineNumberAttr + "$='line']").length != 0) { options.settings.extend.afterRenderGridMethods.renderLineNumber = $.fn.bsgrid.extendAfterRenderGrid.renderLineNumber }
    ...
    if (options.settings.extend.settings.supportGridEdit) {
      ...
      gridObj.deleteRow = function(row) { $.fn.bsgrid.defaults.extend.deleteRow(row, options) };
      gridObj.addNewEditRow = function() { $.fn.bsgrid.defaults.extend.addNewEditRow(options) }
    }
    if (options.settings.extend.settings.supportColumnMove) { options.settings.extend.initGridMethods.initColumnMove = $.fn.bsgrid.extendInitGrid.initColumnMove }
    if (options.settings.extend.settings.fixedGridHeader) {
      ...
      options.settings.extend.afterRenderGridMethods.fixedHeader = function(parseSuccess, gridData, options) { $.fn.bsgrid.extendOtherMethods.fixedHeader(false, options) }
    }
    if ($.trim(options.settings.extend.settings.searchConditionsContainerId) != "") { options.settings.extend.initGridMethods.initSearchConditions = $.fn.bsgrid.extendInitGrid.initSearchConditions }
    if ($("#" + options.gridId + " tfoot td[" + colsProperties.aggAttr + "!='']").length != 0) { options.settings.extend.afterRenderGridMethods.aggregation = $.fn.bsgrid.extendAfterRenderGrid.aggregation }
  };
  $.fn.bsgrid.extendInitGrid.initGridCheck = function(gridId, options) {
    $.fn.bsgrid.getGridHeaderObject(options).each(function(hi) {
      if (options.columnsModel[hi].check == "true") {
        if ($.trim($(this).html()) == "") { $(this).html('<input class="bsgrid_editgrid_check" type="checkbox"/>') }
  ...
  $.fn.bsgrid.extendInitGrid.initSearchConditions = function(gridId, options) {
    ...
    $.fn.bsgrid.getGridHeaderObject(options).each(function(i) {
      ...
      if (index != "" && text != "" && $.trim(params[index]) == "") { params[index] = text }
    });
    for (var key in params) { conditionsHtml.append('<option value="' + key + '">' + params[key] + "</option>") }
    ...
    $("#" + options.settings.extend.settings.searchConditionsContainerId + " select.bsgrid_conditions_select").change(function() { $(this).next("input.bsgrid_conditions_input").attr("name", $(this).val()) }).trigger("change")
  };
  $.fn.bsgrid.extendInitGrid.initColumnMove = function(gridId, options) {
    ...
    headObj.each(function(i) {
      ...
      $(obj).mousedown(function() { bindDownData(obj, i, headLen) });
      $(obj).mousemove(function(e) {
        ...
        var mObj = obj;
        if (i != headLen - 1 && e.clientX - nLeft > -10) { mObj = nObj }
        if ((i != 0 && e.clientX - left < 10) || (i != headLen - 1 && e.clientX - nLeft > -10)) {
          ...
          if (parseInt(newMWidth) > 19 && parseInt(preNewMWidth) > 19) { $(mObj).width(newMWidth).prev().width(preNewMWidth) }
        } else {
          $(mObj).css({ cursor: "default" });
          releaseDownData(obj, i, headLen)
        }
      });
      $(obj).mouseup(function() { releaseDownData(obj, i, headLen) });
      $(obj).mouseout(function(e) {
        e = e || event;
        var objOffect = $(obj).offset();
        if (objOffect.top > e.clientY || objOffect.top + $(obj).height() < e.clientY) { releaseDownData(obj, i, headLen) }
      });

      function bindDownData(obj, i, headLen) {
        if (i != 0) { $(obj).prev().data("ex_mousedown", "mousedown") }
        $(obj).data("ex_mousedown", "mousedown");
        if (i != headLen - 1) { $(obj).next().data("ex_mousedown", "mousedown") }
      }

      function releaseDownData(obj, i, headLen) {
        if (i != 0) { $(obj).prev().data("ex_mousedown", "") }
        $(obj).data("ex_mousedown", "");
        if (i != headLen - 1) { $(obj).next().data("ex_mousedown", "") }
      }
    })
  };
  ...
  $.fn.bsgrid.extendRenderPerColumn.renderForm = function(record, rowIndex, colIndex, tdObj, trObj, options) {
    if (rowIndex < options.curPageRowsNum) {
      ...
      var tdHtml = "&nbsp;";
      if (edit in options.settings.extend.settings.gridEditConfigs) { tdHtml = options.settings.extend.settings.gridEditConfigs[edit].build(edit, value, record, rowIndex, colIndex, tdObj, trObj, options) } else {
        return false
      }
      tdObj.html(tdHtml);
  ...
  $.fn.bsgrid.extendAfterRenderGrid.renderLineNumber = function(parseSuccess, gridData, options) {
    $.fn.bsgrid.getGridHeaderObject(options).each(function(i) {
      ...
      if (num == "line" || num == "total_line") { $.fn.bsgrid.getRows(options).filter(":lt(" + options.curPageRowsNum + ")").find("td:nth-child(" + (i + 1) + ")").each(function(li) { $(this).html((num == "line") ? (li + 1) : (li + options.startRow)) }) }
    })
  };
    ...
    $.fn.bsgrid.getGridHeaderObject(options).each(function(hi) {
      if (options.columnsModel[hi].check == "true") {
        ...
        checkboxObjs.change(function() {
          ...
          if (!checked && allCheckboxObjs.filter(":checked").length == allCheckboxObjs.length) { $.bsgrid.adaptAttrOrProp(checkboxObj, "checked", true) } else {
            if (checked && allCheckboxObjs.filter(":checked").length != allCheckboxObjs.length) { $.bsgrid.adaptAttrOrProp(checkboxObj, "checked", false) }
          }
        })
      }
    })
      ...
      $(this).find("td").each(function(ci) {
        if (columnsModel[ci].edit != "") {
          $(this).find(":input").change(function() {
            ...
            if ($.trim($(this).val()) != value) { $(this).addClass("bsgrid_editgrid_change") } else { $(this).removeClass("bsgrid_editgrid_change") }
            rowObj.data("change", rowObj.find(".bsgrid_editgrid_change").length)
          })
        }
      });
      if (options.settings.extend.settings.supportGridEditTriggerEvent == "") { $(this).find(".bsgrid_editgrid_hidden").each(function() { showCellEdit(this) }) } else {
        if (options.settings.extend.settings.supportGridEditTriggerEvent == "rowClick") { $(this).click(function() { $(this).find(".bsgrid_editgrid_hidden").each(function() { showCellEdit(this) }) }) } else {
          if (options.settings.extend.settings.supportGridEditTriggerEvent == "rowDoubleClick") { $(this).dblclick(function() { $(this).find(".bsgrid_editgrid_hidden").each(function() { showCellEdit(this) }) }) } else {
            if (options.settings.extend.settings.supportGridEditTriggerEvent == "cellClick") {
              $(this).find(".bsgrid_editgrid_hidden").each(function() {
                var formObj = this;
                $(formObj).parent("td").click(function() { showCellEdit(formObj) })
              })
            } else {
              if (options.settings.extend.settings.supportGridEditTriggerEvent == "cellDoubleClick") {
                $(this).find(".bsgrid_editgrid_hidden").each(function() {
                  var formObj = this;
                  $(formObj).parent("td").dblclick(function() { showCellEdit(formObj) })
                })
              }
            }
          }
        ...
        if (aggName == "count") { val = options.curPageRowsNum } else {
          if (aggName == "countnotnone" || aggName == "sum" || aggName == "avg" || aggName == "max" || aggName == "min" || aggName == "concat") {
            ...
            $.fn.bsgrid.getRows(options).filter(":lt(" + options.curPageRowsNum + ")").each(function(ri) {
              ...
              if (rval == "") {} else {
                if (aggName == "countnotnone") { val = (val == null ? 0 : val) + 1 } else {
                  if (aggName == "sum" || aggName == "avg") {
                    if (!isNaN(rval)) { val = (val == null ? 0 : val) + parseFloat(rval) }
                  } else {
                    if (aggName == "max" || aggName == "min") {
                      if (!isNaN(rval) && (val == null || (aggName == "max" && parseFloat(rval) > val) || (aggName == "min" && parseFloat(rval) < val))) { val = parseFloat(rval) }
                    } else {
                      if (aggName == "concat") { valHtml.append(rval) }
                    }
                  }
                }
              }
            });
            if (aggName == "avg" && val != null) { val = val / options.curPageRowsNum } else {
              if (aggName == "concat") { val = valHtml.toString() }
            }
          } else {
            if (aggName == "custom") { val = eval(columnsModel[i].aggIndex)(gridObj, options) }
          }
        }
        val = val == null ? "" : val;
        $(this).html(val)
  ...
  $.fn.bsgrid.defaults.extend.getCheckedRowsIndexs = function(options) {
    var rowIndexs = [];
    $.fn.bsgrid.getRows(options).each(function(i) {
      if ($(this).find("td>input:checked").length == 1) { rowIndexs[rowIndexs.length] = i }
    });
    return rowIndexs
  };
  $.fn.bsgrid.defaults.extend.getCheckedRowsRecords = function(options) {
    var records = [];
    $.each($.fn.bsgrid.defaults.extend.getCheckedRowsIndexs(options), function(i, rowIndex) { records[records.length] = $.fn.bsgrid.getRecord(rowIndex, options) });
    return records
  };
  $.fn.bsgrid.defaults.extend.getCheckedValues = function(index, options) {
    var values = [];
    $.each($.fn.bsgrid.defaults.extend.getCheckedRowsRecords(options), function(i, record) { values[values.length] = $.fn.bsgrid.getRecordIndexValue(record, index, options) });
    return values
  };
  ...
  $.fn.bsgrid.defaults.extend.getChangedRowsIndexs = function(options) {
    ...
    $.fn.bsgrid.getRows(options).each(function(i) {
      ...
      if (!isNaN(cellChangedNumStr) && parseInt(cellChangedNumStr) > 0) { rowIndexs[rowIndexs.length] = i }
    });
    return rowIndexs
  };
  $.fn.bsgrid.defaults.extend.getChangedRowsOldRecords = function(options) {
    var records = [];
    $.each($.fn.bsgrid.defaults.extend.getChangedRowsIndexs(options), function(i, rowIndex) { records[records.length] = $.fn.bsgrid.getRecord(rowIndex, options) });
    return records
  };
  $.fn.bsgrid.defaults.extend.getRowsChangedColumnsValue = function(options) {
    ...
    $.each($.fn.bsgrid.defaults.extend.getChangedRowsIndexs(options), function(i, rowIndex) {
      ...
      $.fn.bsgrid.getRows(options).filter(":eq(" + rowIndex + ")").find("td").each(function(ci) {
        if ($(this).find(".bsgrid_editgrid_change").length > 0) { values["row_" + rowIndex][options.columnsModel[ci].index] = options.settings.extend.settings.gridEditConfigs[options.columnsModel[ci].edit].val($(this).find(".bsgrid_editgrid_change")) }
      })
    });
    return values
  };
  $.fn.bsgrid.defaults.extend.deleteRow = function(row, options) { $.fn.bsgrid.getRow(row, options).remove() };
  $.fn.bsgrid.defaults.extend.addNewEditRow = function(options) {
    ...
    gridObj.getRowCells(0).each(function(colIndex) {
      ...
      if (columnModel.render != "") {
        ...
        if (columnModel.edit != "textarea") { $(this).children().val("") } else { $(this).children().text("") }
    ...
    doExport: function(h, o, d) {
      ...
      for (var f = 0; f < h.length; f++) {
        if (a.trim(h.eq(f).attr(c.colsProperties.exportAttr)) != "false") {
          ...
          if (isNaN(l)) {
            if (l.endWith("px")) { e = parseInt(l.replace("px", "")) } else {
              if (l.endWith("%")) {
                l = l.replace("%", "");
                if (!isNaN(l)) { e = c.colWidthPercentmultiplier * parseInt(l) }
              }
            }
          }
          m = m + "," + e;
          var k = a.trim(h.eq(f).attr(c.colsProperties.alignAttr));
          if (k == "") { k = c.colsProperties.align }
          j = j + "," + k
        }
      }
      var b;
      if ((typeof o).toLowerCase() == "string" || o instanceof String) { b = (o.startWith("&") ? o.substring(1) : o) } else { b = a.bsgrid.param(o, true) }
...
(function(a) {
  a.fn.bsgrid_form = {
    ...
    formObjs: {},
    init: function(e, d) {
      var c = { settings: a.extend(true, {}, a.fn.bsgrid_form.defaults, d), formId: e, jqueryObj: a("#" + e), formType: "" };
      var b = { options: c, addAssistShowFormTags: function() { a.fn.bsgrid_form.addAssistShowFormTags(c) }, showForm: function(f) { a.fn.bsgrid_form.showForm(c, f) }, showOrHideRequireSpan: function(f) { a.fn.bsgrid_form.showOrHideRequireSpan(c, f) }, showOrHideAssistForms: function(f) { a.fn.bsgrid_form.showOrHideAssistForms(c, f) }, showOrHideTag: function(f) { a.fn.bsgrid_form.showOrHideTag(c, f) } };
    ...
    addAssistShowFormTags: function(b) {
      a(".formInput select", b.jqueryObj).each(function() {
        ...
        for (var e = 0; e < d.length; e++) {
          var c = d[e].name;
          if (c.toLowerCase().endWith("able") && a(this).attr(c) == "false") { a(this).prev("input").attr(c, "false") }
        }
      });
      a(".formInput textarea", b.jqueryObj).each(function() { a(this).before('<div class="assist" style="display: none;"></div>') })
    },
    showForm: function(b, c) {
      ...
      if (c.startWith("view")) { a(".formInput :input:not(:button,:submit,:reset)", b.jqueryObj).css({ "border-width": "0" }).attr("readOnly", "readOnly") } else {
        if (c.startWith("add")) { a(".formInput :input:not(:button,:submit,:reset)", b.jqueryObj).css({ border: "solid 1px white" }).removeAttr("readOnly") } else {
          if (c.startWith("edit")) {
            a(".formInput :input:not(:button,:submit,:reset)", b.jqueryObj).css({ border: "solid 1px white" }).removeAttr("readOnly");
            a(".formInput :input[" + c + "Able=false]", b.jqueryObj).css({ "border-width": "0" }).attr("readOnly", "readOnly")
          }
        }
      }
    },
    showOrHideRequireSpan: function(b, c) {
      if (c.startWith("view")) { a(".formLabel span.require", b.jqueryObj).hide() } else {
        if (c.startWith("edit")) { a(".formLabel:has(span.require) ~ .formInput:has(:input[" + c + "Able=false])", b.jqueryObj).prev().find("span.require").hide() } else { a(".formLabel span.require", b.jqueryObj).show() }
      }
    },
    ...
    showOrHideTag: function(b, c) {
      a("*", b.jqueryObj).each(function() {
        ...
        if (d != "") {
          if ((c.startWith("view") || c.startWith("add") || c.startWith("edit")) && ("," + d + ",").indexOf("," + c + ",") > -1) { a(this).show() } else { a(this).hide() }
        }
      })
    }
  }
